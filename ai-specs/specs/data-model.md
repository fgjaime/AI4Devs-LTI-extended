...
- `status`: Current status of the position (required, default: "Draft", valid values: Draft, Open, Contratado, Cerrado, Borrador)
- `isVisible`: Boolean indicating if the position is publicly visible (required, default: false)
...
- Status is required (has default value "Draft" if not provided during creation) and must be one of: Draft, Open, Contratado, Cerrado, Borrador
- isVisible is required (has default value false if not provided during creation)
...
- For PATCH update operations, all fields are optional in the request body
- Only provided fields will be updated; unprovided fields remain unchanged
- If a field is provided, it must meet the validation rules specified above

**Immutable Fields:**
The following fields cannot be updated and will return a 400 error if included in the update request:
- `id`: Position ID is immutable
- `companyId`: Company association cannot be changed after creation
- `interviewFlowId`: Interview flow association cannot be changed after creation

**Validation Rules for Updates:**
- **Title**: If provided, must be a non-empty string with max 100 characters
- **Description**: If provided, cannot be an empty string
- **Location**: If provided, cannot be an empty string
- **Job Description**: If provided, cannot be an empty string
- **Status**: If provided, must be one of: "Draft", "Open", "Contratado", "Cerrado", "Borrador"
- **isVisible**: If provided, must be a boolean value
- **Salary Range**: 
  - `salaryMin` and `salaryMax` must be numbers >= 0 if provided
  - If both `salaryMin` and `salaryMax` are provided, `salaryMax` must be >= `salaryMin`
- **Application Deadline**: If provided, must be a valid ISO 8601 date-time string (e.g., "2026-12-31T23:59:59Z")

**Error Responses:**
- `400 Bad Request`: Invalid ID format, validation errors, or immutable field attempts
- `404 Not Found`: Position with the specified ID does not exist
...
**Update Operations:**
- **Endpoint**: `PATCH /candidates/{candidateId}/interviews/{interviewId}`
- All interview fields are optional in the update request body (partial update support)
- Only provided fields are updated; unprovided fields remain unchanged
- Validation rules for updates:
  - `interviewDate`: If provided, must be a valid ISO 8601 date-time string and parseable as a valid date
  - `interviewStepId`: If provided, must exist and belong to the interview flow associated with the application's position
  - `employeeId`: If provided, must exist and be active (`isActive = true`)
  - `score`: If provided, must be an integer between 0 and 5 (inclusive) or null
  - `notes`: If provided, must be a string with maximum 1000 characters or null
  - `result`: If provided, must be one of `"Pending"`, `"Passed"`, or `"Failed"`, or null

**Delete Operations:**
- **Endpoint**: `DELETE /candidates/{candidateId}/interviews/{interviewId}`
- **Business Rule**: Only pending interviews (result is `null` or `"Pending"`) can be deleted. Completed interviews (result is `"Passed"` or `"Failed"`) cannot be deleted to preserve recruitment history.
- **Request Body**: Required, must include `reason` field (string, non-empty, maximum 500 characters) for audit trail purposes.
- **Validation Rules**:
  - `candidateId`: Required, must be numeric, must exist
  - `interviewId`: Required, must be numeric, must exist, must belong to the specified candidate
  - `reason`: Required, must be a non-empty string, maximum 500 characters
- **Response**: Returns HTTP 200 with success message on successful deletion
- **Error Responses**:
  - `400`: Validation error (invalid ID format, missing/empty reason, reason too long)
  - `404`: Resource not found (candidate or interview not found, or interview does not belong to candidate)
  - `422`: Business rule violation (attempting to delete a completed interview)
  - `500`: Internal server error
- **Audit Trail**: All deletion operations are logged with interview ID, candidate ID, deletion reason, and timestamp for compliance purposes.
